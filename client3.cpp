...
#pragma pack(pop)
	...
	public:
		...
		client(int thread_count, char const* host, char const* port, uint32_t total_count, size_t session_count)
			: thread_count_(thread_count)
		{
			...
			for (size_t i = 0; i < session_count; ++i)
			{
				...
				std::unique_ptr<session> new_session(new session(io_service, total_count));
		...
		~client()
		{
			...
			std::cout << "average time(us): " << (uint64_t)((double)total_time / finished_count) << "\n";
		}
