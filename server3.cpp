...
#pragma pack(push,1)
	struct Header
	{
		...
		uint32_t get_body_size() const { return ntohl(body_size_); }
		uint32_t get_full_size() const { return sizeof(Header) + ntohl(body_size_); }
...
#pragma pack(pop)
	...
	public:
		...
		void write()
		{
			...
			asio::async_write(socket_, asio::buffer(buffer_, full_size),
				[this, self = shared_from_this()](const asio::error_code& err, size_t cb)
			{
				if (!err)
				{
					if (check_count())
		...
		void read_body()
		{
			...
			asio::async_read(socket_, asio::buffer(buffer_ + sizeof(Header), body_size),
				[this, self = shared_from_this()](const asio::error_code& err, size_t cb)
			{
				if (!err)
				{
	...
	public:
		...
		server(int thread_count, const asio::ip::tcp::endpoint& endpoint, uint32_t total_count)
		...
		void wait() { service_pool_.run(); }

	private:

		void accept()
		{
			std::shared_ptr<session> new_session(new session(service_pool_.get_io_service(), total_count_));
			auto& socket = new_session->socket();

			acceptor_.async_accept(socket,
				[this, new_session = std::move(new_session)](const asio::error_code& err)
			{
				if (!err)
				{
					new_session->start();
...
{
	auto endpoint = asio::ip::tcp::endpoint(asio::ip::address::from_string(host), atoi(port));
