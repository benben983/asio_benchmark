...
namespace
{
	...
	public:
		...
		void add(bool error, size_t count_written, size_t count_read, size_t bytes_written, size_t bytes_read)
		{
		...
		void print()
		{
			...
			std::cout << static_cast<double>(total_bytes_read_)	   / (timeout_ * 1024 * 1024) << " MiB/s read  throughput\n";
			std::cout << static_cast<double>(total_bytes_written_) / (timeout_ * 1024 * 1024) << " MiB/s write throughput\n";
		}
	...
	public:
		...
		size_t count_written() const		{ return count_written_; }
		size_t count_read() const			{ return count_read_; }
	...
	public:
		...
		client(int thread_count, char const* host, char const* port, size_t block_size, size_t session_count, int timeout)
		...
		~client()
		{
			...
			{
				stats_.add(session->error(),session->count_written(), session->count_read(),session->bytes_written(), session->bytes_read());
			}
