...
namespace
{
	...
	public:
		...
		void write(char* buffer, size_t len)
		{
			socket_.async_write_some(asio::buffer(buffer, len),
				[this, self = shared_from_this(), buffer](const asio::error_code& err, size_t cb)
			{
	...
	private:
		...
		void accept()
		{
			std::shared_ptr<session> new_session(new session(service_pool_.get_io_service(), block_size_));
