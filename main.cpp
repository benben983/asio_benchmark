...
void client_test1(int thread_count, char const* host, char const* port, size_t block_size, size_t session_count, int timeout);

void client_test2(int thread_count, char const* host, char const* port, size_t block_size, size_t session_count, int timeout);

void server_test1(int thread_count, char const* host, char const* port, size_t block_size);

void server_test2(int thread_count, char const* host, char const* port, size_t block_size);

void socket_pair_test(size_t pair_count, size_t active_count, size_t write_count);

void client_test3(int thread_count, char const* host, char const* port, size_t total_count, size_t session_count);

void server_test3(int thread_count, char const* host, char const* port, size_t total_count);
...
int usage()
{
	std::cerr << "Usage: asio_test socketpair <pair_count> <active_count> <write_count>\n";

	std::cerr << "Usage: asio_test client1 <host> <port> <threads> <blocksize> <sessions> <time>\n";
	std::cerr << "Usage: asio_test server1 <address> <port> <threads> <blocksize>\n";

	std::cerr << "Usage: asio_test client2 <host> <port> <threads> <blocksize> <sessions> <time>\n";
	std::cerr << "Usage: asio_test server2 <address> <port> <threads> <blocksize>\n";

	std::cerr << "Usage: asio_test client3 <host> <port> <threads> <totalcount> <sessions>\n";
	std::cerr << "Usage: asio_test server3 <address> <port> <threads> <totalcount>\n";
...
void change_limit()
{
	...
	if (rlim.rlim_cur < rlim.rlim_max)
	{
		...
		{
			std::cout << "setrlimit failed, error: " << strerror(errno) << " " << std::to_string(old_cur) + " to " << std::to_string(rlim.rlim_max) << "\n";
		}
		else
		{
			std::cout << "setrlimit success: " << std::to_string(old_cur) << " to " << std::to_string(rlim.rlim_max) << "\n";
		}
	}
}
#endif
